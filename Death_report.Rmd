---
title: "Live Doublig-time of Deaths and R0 for deaths due to COVID-19"
author: "Debangan Dey"
date: "4/23/2020"
...
dt.df %>% filter(state %in% state.int) %>% ggplot(aes(x=min_time,y=log10(mean), group=state,color=state)) + geom_line() + 
  geom_dl(aes(label = state), method = list(dl.trans(x = x - 1, y= y+0.1),dl.combine("last.points"), cex = 1))  + xlim (c(0,50)) + ylim(c(0,2)) + 
...
dt.us.df %>% filter(state %in% state.int.us) %>% ggplot(aes(x=min_time,y=log(mean), group=state,color=state))+ geom_line() + 
  geom_dl(aes(label = state), method = list(dl.trans(x = x - 1, y= y+0.1),dl.combine("last.points"), cex = 1))  + xlim(c(0,40)) +
